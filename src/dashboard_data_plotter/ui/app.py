...
class DashboardDataPlotter(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(btns, text="Save All…", command=self.save_all_datasets).grid(row=0, column=4, padx=(6, 0))
        ...
        self.files_tree.heading("name", text="Dataset", command=self.sort_by_dataset_name)
        ...
        ttk.Label(angle_frame, text="Plot type:").grid(row=0, column=0, sticky="w")
        ...
        ttk.Radiobutton(pt, text="Bar (avg per dataset)", variable=self.plot_type_var, value="bar",
                        command=self._on_plot_type_change).grid(row=1, column=0, sticky="w")

        ttk.Label(angle_frame, text="Angle column:").grid(row=1, column=0, sticky="w")
        ...
        self.close_loop_chk = ttk.Checkbutton(left, text="Close loop (connect 360°)", variable=self.close_loop_var)
        ...
        ttk.Entry(sentinel_frame, textvariable=self.sentinels_var, width=32).grid(row=0, column=1, sticky="w", padx=(8, 0))
        ...
        ttk.Radiobutton(vm_frame, text="Absolute metric values", variable=self.value_mode_var, value="absolute").grid(row=0, column=0, sticky="w")
        self.rb_percent_mean = ttk.Radiobutton(vm_frame, text="% of dataset mean", variable=self.value_mode_var, value="percent_mean")
        ...
        ttk.Label(left, textvariable=self.status, wraplength=380, foreground="#333").grid(row=20, column=0, sticky="w", pady=(10, 0))
    ...
    def _set_plot_type_controls_state(self):
        ...
        try:
            self.angle_combo.configure(state="disabled" if is_bar else "readonly")
        except Exception:
            pass
        try:
            self.close_loop_chk.configure(state="disabled" if is_bar else "normal")
        except Exception:
            pass
    ...
    def _on_plot_type_change(self):
        ...
        if hasattr(self, "rb_percent_mean"):
            self.rb_percent_mean.configure(state=("disabled" if is_bar else "normal"))
    ...
    def sort_by_dataset_name(self):
        ...
        items.sort(
            key=lambda iid: dataset_sort_key(self.files_tree.item(iid, "values")[1]),
            reverse=reverse,
        )
    ...
    def get_plot_order_source_ids(self):
        try:
            ids = [iid for iid in self.files_tree.get_children("") if iid in self.loaded]
            if ids:
                return ids
        except Exception:
            pass
    ...
    def load_from_paste(self):
        ...
        try:
            ...
            messagebox.showerror("Paste load error", f"{type(e).__name__}: {e}")
            return
        ...
        for name, records in datasets:
            if not isinstance(records, list) or (len(records) > 0 and not isinstance(records[0], dict)):
                continue
            display = make_unique_name(str(name), set(self.display_to_id.keys()))
    ...
    def plot(self):
        ...
        if plot_type == "bar":
            ...
            if compare:
                b_ang_deg, b_val = prepare_angle_value(self.loaded[baseline_id], angle_col or "leftPedalCrankAngle", metric_col, sentinels)
            ...
            for sid in ordered:
                ...
                try:
                    ang_deg, val = prepare_angle_value(self.loaded[sid], angle_col or "leftPedalCrankAngle", metric_col, sentinels)
                    val2, _ = self._apply_value_mode(val, value_mode)
                    mval = float(np.nanmean(val2))
                    heights.append(0.0 if compare and sid == baseline_id else (mval - baseline_mean if compare else mval))
                    labels.append(label)
                except Exception as e:
                    errors.append(f"{label}: {e}")

            if not labels:
                messagebox.showinfo("Nothing to plot", "No datasets produced valid bar values.")
                self._redraw_empty()
                return

            x = np.arange(len(labels))
            self.ax.axhline(0.0, color="red" if compare else "black", linewidth=1.2)
            ...
            if compare:
                ...
                self.ax.set_title(f"Mean {metric_col} difference vs baseline {b_label} ({mode_str})")
                self.ax.set_ylabel("Difference vs baseline")
            else:
                self.ax.set_title(f"Mean {metric_col} per dataset ({mode_str})")
        ...
        if not compare:
            for sid in self.get_plot_order_source_ids():
                ...
                label = self.id_to_display.get(sid, os.path.basename(sid))
                try:
                    ang_deg, val = prepare_angle_value(self.loaded[sid], angle_col, metric_col, sentinels)
                    val2, _ = self._apply_value_mode(val, value_mode)
            ...
            try:
                b_ang_deg, b_val = prepare_angle_value(self.loaded[baseline_id], angle_col, metric_col, sentinels)
                b_val2, _ = self._apply_value_mode(b_val, value_mode)
            except Exception as e:
            ...
            if not deltas_by_id:
                messagebox.showinfo("Nothing to plot", "No non-baseline datasets produced valid comparison traces.")
                self._redraw_empty()
                return
            ...
            mode_str = "absolute" if value_mode == "absolute" else "% of mean"
            self.ax.set_title(f"{metric_col} ({mode_str}) difference to Baseline ({b_label})", pad=18)
            self.ax.grid(True)
            self.ax.legend(loc="upper left", bbox_to_anchor=(1.02, 1.05), fontsize=9, frameon=False)
            self.ax.set_position([0.02, 0.08, 0.8, 0.8])
